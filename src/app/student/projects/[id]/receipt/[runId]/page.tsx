...
import { db } from "@/db";
import { profiles, reasoningReceipts, agentRuns, projects } from "@/db/schema";
...
export default async function ReceiptPage({
  ...
  const [receipt] = await db
    ...
    .where(and(eq(reasoningReceipts.runId, runId), eq(reasoningReceipts.projectId, id)));
  ...
  const runDate = new Date(receipt.createdAt);
  ...
  const knowledgeUsed = (receipt.knowledgeUsed as string[]) || [];
  const rulesApplied = (receipt.rulesApplied as string[]) || [];
  const stepsFollowed = (receipt.stepsFollowed as string[]) || [];
  const toolsUsed = (receipt.toolsUsed as string[]) || [];
  const approvalRequired = (receipt.approvalRequired as string[]) || [];
  const safetyFlags = (receipt.safetyFlags as string[]) || [];
        ...
        <div style={{ filter: "drop-shadow(0 18px 50px rgba(58,46,28,.15))" }}>
          ...
          <div
            style={{
              ...
              background: "#2A2A3C",
              maskImage:
                "repeating-linear-gradient(90deg, transparent 0px, transparent 8px, white 8px, white 16px)",
              WebkitMaskImage:
                "repeating-linear-gradient(90deg, transparent 0px, transparent 8px, white 8px, white 16px)",
            }}
          />
          ...
          >
            {/* AI Worker */}
            <ReceiptRow label="AI WORKER" value={project?.name || "Unknown"} />
            <ReceiptDots />

            {/* Goal */}
            <ReceiptRow label="GOAL" value={receipt.goal || "—"} />
            <ReceiptDots />
            ...
            <div>
              ...
              {knowledgeUsed.length > 0 ? (
                knowledgeUsed.map((k, i) => (
                  <div key={i} className="mt-1" style={{ color: "#2A2A3C" }}>
                    · {k}
                  </div>
                ))
              ) : (
            ...
            {/* Tools Used + Approval Required — 2-up */}
            <div className="grid grid-cols-2 gap-4">
              <div>
                <div style={{ color: "#8A8071" }}>TOOLS USED</div>
                <div style={{ color: "#2A2A3C" }}>
              ...
              <div>
                ...
                <div style={{ color: "#2A2A3C" }}>
                  {approvalRequired.length > 0 ? approvalRequired.join(", ") : "None"}
                </div>
              </div>
            </div>
            <ReceiptDots />

            {/* Safety Flags + Model — 2-up */}
            <div className="grid grid-cols-2 gap-4">
              <div>
              ...
              <>
                ...
                <div>
                  ...
                  <div className="mt-1" style={{ color: "#2A2A3C", lineHeight: 1.6 }}>
                    {(receipt.output as string).slice(0, 300)}
                    {(receipt.output as string).length > 300 ? "…" : ""}
                  </div>
                </div>
              </>
            )}
          ...
          <div
            style={{
              ...
              background: "#FFFFFF",
              maskImage:
                "repeating-linear-gradient(90deg, white 0px, white 8px, transparent 8px, transparent 16px)",
              WebkitMaskImage:
                "repeating-linear-gradient(90deg, white 0px, white 8px, transparent 0px, transparent 16px)",
            }}
          />
          <div
            className="rounded-b-card py-4 text-center"
            style={{ background: "#FBF6EC" }}
          >
            <div className="font-mono text-[10px] uppercase tracking-widest" style={{ color: "#8A8071" }}>
              AGENT FACTORY JUNIOR · POWERED BY MOCK MODEL
            </div>
          </div>
        </div>
