import type { agentRuns, projects, replays } from "@/db/schema";

type ReplayCardProps = {
  replay: typeof replays.$inferSelect;
  run: typeof agentRuns.$inferSelect | undefined;
  project: typeof projects.$inferSelect | undefined;
};

export function ReplayCard({ replay, run, project }: ReplayCardProps) {
    ...
    <div style={{ filter: "drop-shadow(0 18px 50px rgba(58,46,28,.15))" }}>
      ...
      >
        ...
        <div className="font-mono text-xs" style={{ color: "#8A8071" }}>
          RUN #{replay.runId.slice(0, 8).toUpperCase()} · {dateStr.toUpperCase()} · {timeStr}
        </div>
      </div>
      ...
      >
        ...
        <div>
          ...
          {knowledgeUsed.length > 0 ? (
            knowledgeUsed.map((k, i) => <div key={i} className="mt-1" style={{ color: "#2A2A3C" }}>· {k}</div>)
          ) : (
        ...
        <div>
          ...
          {stepsFollowed.length > 0 ? (
            stepsFollowed.map((s, i) => <div key={i} className="mt-1" style={{ color: "#2A2A3C" }}>{i + 1}. {s}</div>)
          ) : (
            <div style={{ color: "#8A8071" }}>None</div>
          )}
        </div>
